...
// ── PATCH: editar nombre de usuario y/o grado ──────────────────
async function editar(request, context, user) {
  try {
    ...
    if (body.grado !== undefined) {
      patch.grado = body.grado ? String(body.grado).trim() : null;
    }
    if (body.username !== undefined) {
      const username = String(body.username).trim();
      if (!username) {
        return NextResponse.json(
          { error: 'El nombre de usuario no puede estar vacío', code: 'INVALID' },
          { status: 400 }
        );
      }
      // Único (excluyendo al propio estudiante).
...
async function eliminar(request, context, user) {
  try {
    ...
    // Las FK con ON DELETE CASCADE borran inscripciones, FSRS, sesiones, etc.
    const { data, error } = await supabase
      .from('usuario')
      .delete()
      .eq('id_usuario', id)
...
export const PATCH  = withAuth(editar,   'docente');
export const DELETE = withAuth(eliminar, 'docente');
