...
OBLIGATORIO — NO REPETIR: la nueva pregunta DEBE ser CLARAMENTE DISTINTA de la PREGUNTA ACTUAL y de las que ya existen (listadas abajo). NO la copies ni la parafrasees ni cambies solo un número o una palabra: enfócala en OTRO aspecto, subtema o ángulo del tema.
${ajusteTxt}${instruccionTxt}${clarasTxt}${idiomaTxt}${matematicasTxt}
PREGUNTA ACTUAL (la que se va a reemplazar, NO la repitas): ${currentQuestion}${exclusion}
 ...
 */
export async function gradeAnswer({ question, referenceAnswer, studentResponse, bloomLevel, esIngles = false }) {
  const model = getModel(0.2, 1024);
  const reglaOrtografia = esIngles
    ? `- ORTOGRAFÍA (materia de INGLÉS): sé indulgente con la ortografía y las tildes del texto en ESPAÑOL que enmarca la respuesta. PERO la PALABRA o FRASE EN INGLÉS que se está aprendiendo (p. ej. "an", "well", "happily", "so") SÍ debe estar bien escrita: si el alumno la escribe mal o inventa una forma que no existe (p. ej. "fastly"), eso es un error real y baja la nota. No confundas dar la traducción al español con dar la palabra en inglés pedida.`
    : `- ORTOGRAFÍA: sé MUY indulgente con la ortografía, las tildes, las mayúsculas y la notación. NO se está evaluando cómo escribe el alumno, sino que sepa el CONCEPTO. Palabras mal escritas ("advervios", "uan función"), tildes faltantes o notación imprecisa ("5x" en vez de "5") NO deben bajar la nota si la idea es correcta.`;
...
CÓMO PUNTUAR (score de 0.0 a 1.0) — MÉTODO DE RÚBRICA, A FAVOR DE LO QUE EL ESTUDIANTE SÍ ENTENDIÓ:
1) Descompón la REFERENCIA en sus IDEAS CLAVE (los conceptos que hacen correcta la respuesta).
2) Cuenta cuántas de esas ideas clave aparecen en la respuesta del estudiante (aunque las diga con sus propias palabras). El score base ≈ fracción de ideas clave cubiertas.
3) Aplica estos ajustes:
   - Si el estudiante captó la IDEA CENTRAL, garantiza al menos nota media aunque le falten detalles.
   - Si escribió MÁS de lo pedido pero la respuesta correcta SÍ está dentro, cuéntala como correcta: el texto extra o de contexto NO baja la nota; solo bajan la nota las partes realmente equivocadas.
   - NO exijas justificación o explicación si la PREGUNTA no la pide. Si solo pide identificar o nombrar algo y el estudiante lo nombra bien, es nota alta aunque no explique el porqué.
   - Un detalle sobrante equivocado descuéntalo un poco, NO hunde una respuesta cuyo núcleo es correcto.
${reglaOrtografia}

TOPES (para no premiar de más):
- PROFUNDIDAD: si la PREGUNTA pide EXPLICAR, JUSTIFICAR, DESCRIBIR, COMPARAR o razonar (o el nivel Bloom es alto) y el estudiante responde correcto pero DEMASIADO BÁSICO / superficial (sin el razonamiento pedido), NO le des nota alta: tópalo en nota media (score máximo ≈ 0.6). Si la pregunta solo pedía identificar/nombrar, esto NO aplica.
- RAZONAMIENTO EQUIVOCADO: si el estudiante acierta el DATO pero lo acompaña de un razonamiento o justificación explícitamente INCORRECTA, tópalo en nota media (score máximo ≈ 0.6), aunque el dato esté bien.

PENALIZA DE VERDAD (score bajo aunque use palabras parecidas): definiciones invertidas, negaciones, conceptos intercambiados o una respuesta central equivocada.
...
- EMPIEZA SIEMPRE reconociendo lo que hiciste bien (lo bueno de tu respuesta) antes de señalar lo que faltó o estuvo mal. Si algo estuvo mal, explícalo con claridad pero sin regañar.
- CASO ESPECIAL — si el estudiante dice que NO SABE, pide que le expliquen, deja la respuesta casi vacía o muestra confusión/frustración (p. ej. "no sé", "no entiendo", "explícame", "ayuda"): NO lo trates como un error ni lo critiques. Responde con EXTRA calidez y ánimo, dile que está bien no saberlo todavía, y enséñale el concepto desde cero de forma sencilla y motivadora. En este caso el diagnóstico debe ser comprensivo, no correctivo.
- diagnosis: primero lo que hiciste bien; luego, con tacto, dónde está tu confusión (ej. "Vas bien en... pero confundiste...").
- explanation: tu punto de mejora concreto, qué reforzar (ej. "Recuerda que...").
- example: un ejemplo claro que te ayude a entenderlo.
