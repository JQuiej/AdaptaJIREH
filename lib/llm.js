...
const AJUSTE_MATEMATICAS =
  'AJUSTE DE DIFICULTAD (MATEMÁTICAS): baja NOTORIAMENTE la dificultad, ya que el nivel actual resulta demasiado alto para los alumnos. ' +
  'Usa SIEMPRE números pequeños y "redondos" (de preferencia enteros y de una o dos cifras), un ÚNICO paso o cálculo por pregunta (nunca problemas de varios pasos encadenados), ' +
  'enunciados muy cortos y directos, vocabulario sencillo y ejemplos cotidianos y concretos (dinero, objetos, edades, etc.). ' +
  'Evita fracciones complicadas, decimales largos, potencias, raíces o formulaciones abstractas salvo que el tema lo exija. ' +
...
Reglas: reference_answer = 2-3 oraciones; feedback_hint orienta sin revelar la respuesta.${esMatematicas ? `\n${AJUSTE_MATEMATICAS}` : ''}${exclusionBlock}
Material: ${extractedText.slice(0, MAX_CONTEXT_CHARS)}
Responde SOLO con este JSON:
{"items":[{"bloom":1,"question":"","reference_answer":"","feedback_hint":""}]}`;
...
${ajusteTxt}${instruccionTxt}${idiomaTxt}${matematicasTxt}
