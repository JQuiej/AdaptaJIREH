...
/** TO — Tasa de Olvido. */
export function forgettingRate(prevR, newR) {
  return r3(Math.max(0, Math.min(1, prevR - newR)));
}

/** ELC — Estado Latente de Conocimiento de una unidad. */
export function unitELC(retrievabilities) {
  if (!retrievabilities.length) return 0;
  return r3(retrievabilities.reduce((a, b) => a + b, 0) / retrievabilities.length);
}
